Loop {
    if sd_ImgSearch("Chapters\nightmaremode.png", 30)[1] = 0 { ; switch tabs to nightmare mode
        MouseMove(sd_ImgSearch("Chapters\nightmaremode.png", 10)[2] - 21, sd_ImgSearch("Chapters\nightmaremode.png", 10)[3] + 29), MouseMove(sd_ImgSearch("Chapters\nightmaremode.png", 10)[2] - 20, sd_ImgSearch("Chapters\nightmaremode.png", 10)[3] + 30)
...
JoinChar() {
    ...
    if sd_ImgSearch("Chapters\nm1.png", 10)[1] = 0 { ; if this succeeds, move the mouse here and join
        MouseMove(sd_ImgSearch("Chapters\nm1.png", 10)[2] + 699, sd_ImgSearch("Chapters\nm1.png", 10)[3] + 89), MouseMove(sd_ImgSearch("Chapters\nm1.png", 10)[2] + 700, sd_ImgSearch("Chapters\nm1.png", 10)[3] + 90)
        Send "{Click}"
        Sleep 5000
    ...
    } else if (sd_ImgSearch("Chapters\nightmarebottom.png", 10)[1] = 1) { ; scroll down and try again
        MouseMove((windowX + (windowWidth//2)), (IsSet(offsetY) ? ((windowY + offsetY) + ((windowHeight - offsetY)//2)) : (windowY + (windowHeight//2))))
        Send "{WheelDown}"
        return 0
    } else { ; if nightmare 5 is found, scroll all the way back up and try again
        MouseMove((windowX + (windowWidth//2)), (IsSet(offsetY) ? ((windowY + offsetY) + ((windowHeight - offsetY)//2)) : (windowY + (windowHeight//2))))
        Loop {
            if sd_ImgSearch("Chapters\nightmaretop.png", 10)[1] = 0 {
                break
