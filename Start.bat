...
if exist "submacros\update_checker.ahk" (
	run "" "%~dp0submacros\update_checker.ahk"
	echo %grey%Checking for updates . . .
	<nul set /p "=%green%Press any key to continue%reset%"
		pause >nul
else (
	echo %red%'update_checker.ahk' is missing! This file contains the macro's auto-update functions and you will have to manually download releases without it!%reset%
	<nul set /p "=%red%Press any key to continue%reset%"
		pause >nul
)
