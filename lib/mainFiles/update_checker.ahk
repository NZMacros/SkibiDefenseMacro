QueryUpdateValidity() {
	if Ver2Num(CRRN) > Ver2Num(CurrentVersion) {
	    ...
	    MsgBox "No updates found! You are on the latest version (" VersionID ").", "No Updates Found", "T60"
	}
}
...
Ver2Num(Ver) {
    ...
    global Ver := (MainVer * 100000) + (MajorVer * 10000) + (MidVer * 1000) + (MinorVer * 100) + (VerType * 10) + VerPatch
    return Ver
}

QueryUpdate() {
    confirmation := MsgBox("An updated version of the macro was found. This release is " RRN ", and your current version is " VersionID ". Would you like to download it?", "New Update Available", 0x1) ; Set the user's answer to a query asking them to update
    if confirmation = "OK" {
        Upd2Ver(RRN)
    }
}
