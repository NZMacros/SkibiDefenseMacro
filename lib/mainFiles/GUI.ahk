...
global Code := IniRead(A_SettingsWorkingDir "main-config.ini", "Settings", "Code")
...
sd_MainTabsChange(value) {
	...
	; MainGUI["AdvancedGUICustomisation"].Enabled := value
...
sd_Code(*) {
	global
	Code := MainGUI["Code"].Value
	IniWrite(Code, A_SettingsWorkingDir "main-config.ini", "Settings", "Code")
	if Code = 467854 {
		MainGUI["Code"].Enabled := 0
		MsgBox("bastard", "bastard", 0x1030)
		IniWrite(010000, A_SettingsWorkingDir "main-config.ini", "Settings", "Code")
		IniWrite(1, A_SettingsWorkingDir "main-config.ini", "Settings", "Rudeness")
		ExitApp
	}
}